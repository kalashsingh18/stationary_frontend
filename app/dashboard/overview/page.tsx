...
import type { School, Product, Invoice, Commission } from "@/lib/types"
...
export default function OverviewPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        setLoading(true)
        const [schoolsData, productsData, invoicesData, commissionsData] = await Promise.all([
          getSchools(),
          getProducts(),
          getInvoices(),
          getCommissions(),
  ...
  const monthSales = invoices.reduce((sum, i) => sum + i.totalAmount, 0) // Simplified: Assuming all fetched invoices are "current view" or handling date filtering in real app
  ...
  // Chart Data
  // Monthly Sales Trend (Mock logic for now based on available invoice data grouping)
  const salesData = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"].map(m => {
      let sales = 0;
      invoices.forEach(inv => {
          const d = new Date(inv.date)
           if (d.toLocaleString('default', { month: 'short' }) === m) {
               sales += inv.totalAmount
           }
      })
      return { month: m, sales }
  }).filter(d => d.sales > 0) 
  // If empty, show some placeholders or empty state? Let's leave it dynamic.
  
  const schoolSalesData = schools
    .filter((s) => s.status === "active")
    .map((s) => {
        // Calculate sales from invoices if needed, or use s.totalSales from API
        // Using API value is faster if backend maintains it
        return { name: s.name.split(" ")[0], sales: s.totalSales }
    })
      ...
      <div className="flex flex-col gap-6 p-6">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          <StatCard
            ...
            icon={IndianRupee}
            trend={{ value: "12%", positive: true }} // Placeholder trend
          />
          <StatCard
            title="Month's Sales"
            ...
            icon={TrendingUp}
            trend={{ value: "8.2%", positive: true }} // Placeholder trend
          />
          <StatCard
          ...
          <Card className="lg:col-span-4">
            ...
            <CardContent>
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={salesData.length > 0 ? salesData : [{month: 'No Data', sales: 0}]}>
